<template>
  <main class="main-app">
    <section class="main-app__block" v-if="currentStep === 0">
      ...
      <form novalidate class="form" @submit.prevent="validateStep(['email', 'typePerson'])">
        <BaseInput id="email" title="Endereço de e-mail" type="email" v-model="formData.email" :error="errors.email" />
        <InputRadioGroup text="type-person" :list="typePersonItems" v-model="formData.typePerson"
    ...
    <section class="main-app__block" v-if="currentStep === 1 && formData.typePerson === 'pf'">
      ...
      <form novalidate class="form" @submit.prevent="validateStep(['name', 'cpf', 'birthDate', 'phone'])">
    ...
    <section class="main-app__block" v-if="currentStep === 1 && formData.typePerson === 'pj'">
      ...
      <form novalidate class="form" @submit.prevent="validateStep(['legalName', 'cnpj', 'openingDate', 'phone'])">
    ...
    <section class="main-app__block" v-if="currentStep === 2">
      ...
      <form novalidate class="form" @submit.prevent="validateStep(['password'])">
        <BaseInput id="user" type="text" v-show="false" autocomplete="username" />
    ...
    <section class="main-app__block" v-if="currentStep === 3">
      ...
      <form novalidate class="form" @submit.prevent="submitForm()">
        ...
        <template v-if="formData.typePerson === 'pf'">
          <BaseInput id="name-review" title="Nome" v-model="formData.name" :error="errors.name" autocomplete="username" />
        ...
        <template v-if="formData.typePerson === 'pj'">
          <BaseInput id="legal-name-review" title="Razão social" v-model="formData.legalName"
            :error="errors.legalName" autocomplete="username" />
        ...
        <BaseInput id="phone-review" title="Telefone" v-model="formData.phone" :error="errors.phone" mask="phone" autocomplete="phone" />
        ...
        <div class="button-group">
          ...
          <BaseButton :text="loadingUser ? 'Carregando' : 'Continuar'" :disabled="loadingUser" type="submit" />
        </div>
      </form>
    </section>
  </main>
